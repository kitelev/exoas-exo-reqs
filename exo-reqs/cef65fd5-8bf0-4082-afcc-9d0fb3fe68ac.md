...
⛤ **`sh:Info` добавлен после замера** (реализация, 2026-08-20): `Severity` — закрытый union из **трёх**, и `sh:Info` — такой же не-отказ, как `sh:Warning`. Первая редакция требования его не называла, потому что писалась до чтения типа.

⛔ **Полярность множества — перечислять НЕ-отказы, а не отказ.** Тогда расширение `Severity` попадает на `earl:failed`, то есть потребитель, гейтящийся на отказах, ошибается в сторону **ложной тревоги**, а не молчания. Обратная запись («перечислить отказ, всё прочее мягко») ошибается тихо — худшее направление для гейта.
