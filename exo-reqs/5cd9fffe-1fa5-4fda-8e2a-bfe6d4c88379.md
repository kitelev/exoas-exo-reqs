...
- ⛔ **Не чинить dual-IRI дефект `isEffortBlocked`** (см. «Известные границы»). Перенос — **предусловие** его починки: после переноса один фикс в core лечит обе поверхности разом.
  - ⛤ **Коррекция 2026-08-15 (показана, не переписана молча): хвост «⇒ свой req» был НЕВЕРЕН.** Починка ушла **bug-fix'ом, расширяющим `@req:d6cd2371`** (PR #4055), а не новым требованием. Ошибка была в том, что маршрут выведен из **свойства изменения** («меняет то, что видит пользователь»), а дискриминатор `/feature-sdd` Step 0 ключуется на **чужой спеке**: `d6cd2371` активен и его Gherkin прямо требует «When … блокер завершён Then спека НЕ участвует», плюс его `covers` поимённо называет `BlockerHelpers.isEffortBlocked`. Обратный tell («активный req ТРЕБУЕТ текущего поведения и ОТКЛАДЫВАЕТ изменение») здесь не применим — `d6cd2371` не делает ни того, ни другого. ⇒ читать **Gherkin** активного соседа, а не рассуждать о природе своей правки.
...
- ⛔ **CLI унаследует живой дефект `isEffortBlocked`, и это НАМЕРЕННО — в этом и состоит паритет.** Предикат сравнивает статус блокера со строкой `ems__EffortStatusDone`, но `exocortex-cli` пишет статус **голым `[[uid]]`**; после strip'а брекетов остаётся UID, сравнение не срабатывает, и **завершённый блокер считается активным**. Замер 2026-08-15 на живых данных: у 49 из 58 одиночных блокеров статус в bare-UID-форме; Это класс dual-IRI, лечится матчем **обеих** форм.
  - ⛤ **Коррекция 2026-08-15: «8 эффортов прямо сейчас» — ПРОТУХШЕЕ число.** Оно снято в 14:00; тот же скрипт в 20:45 даёт **1**. Vault сдвинулся, метод — нет. Стабильная величина — **структурная**: 49 из 49 статусованных блокеров в bare-UID-форме ⇒ предикат был сломан для **100 %** реальных блокеров, а видимых 🚩 мало лишь потому, что блокер редко бывает завершён. Закрыто в PR #4055 как bug-fix `@req:d6cd2371`.
